...
namespace xglm {
	...
	{
		mGuiState.lastkeys[0] = mGuiState.key      = key;
		mGuiState.lastkeys[1] = mGuiState.keychar  = key;
		mGuiState.lastkeys[2] = mGuiState.modifier = modifier;
	}

	void ImGUI::onSpecial(unsigned char key, int modifier, int x, int y)
	{
		mGuiState.lastkeys[1] = mGuiState.key      = MapGLUTSpecialKey(key);
		mGuiState.lastkeys[2] = mGuiState.modifier = modifier;
	}
	...
	void ImGUI::onMouse(int button, int state, int x, int y)
	{
		...
		{	//这里只处理鼠标**左**键
			mGuiState.buttondown = (state==IMGUI_MOUSE_DOWN);
			mGuiState.mousex = x;
			mGuiState.mousey = y;
			if( mGuiState.buttondown ) {
				//printf("mousedown (%d, %d)\n", x, y);
				mGuiState.clickxy[0] = x;
				mGuiState.clickxy[1] = y;
			}
		}
		else if ( button==3 )
	...
	int ImGUI::button(int id, int x, int y, int w, int h, char label[])
	{
		...
		if (hitRect(x, y, w, h))
		{
			...
			if (mGuiState.activeitem == 0 && mGuiState.buttondown)
	...
	int ImGUI::slider(int id, int x, int y, int w, int h, double min, double max, double delta, double * value)
	{
		...
		if (mGuiState.kboarditem == 0)
			mGuiState.kboarditem = id;

		// render the bar
		if (mGuiState.kboarditem == id) {
		...
		if (mGuiState.kboarditem == id)
		{
			...
			case IMGUI_RETURN:
				...
				if (mGuiState.modifier & IMGUI_SHIFT)
					mGuiState.kboarditem = mGuiState.lastkbitem;
		...
		if (mGuiState.activeitem == id) {
			...
			mGuiState.kboarditem = id; // let it accept keyboard
			if (*value != newvalue ) {
				*value = newvalue;
				return 1;
			}
	...
	int ImGUI::editbox(int id, int x, int y, int w, int h, char textbuf[], int maxbuf)
	{
		...
		if ( mGuiState.kboarditem == id && ( getTicks() >> 18) & 1)
	...
	int ImGUI::listbox (int id, int x, int y, int w, int h, char*items[], int nitem, int *firstitem, int *selection)
	{
		...
		if( needslider && hitRect(x,y,w,h) )
			mGuiState.kboarditem = 0; // force slider take keyboard
	...
	int ImGUI::listitem(int id, int x, int y, int w, int h, char label[], int selected)
	{
		...
		if (mGuiState.hotitem == id	&& mGuiState.buttondown )
			selected = 1;
