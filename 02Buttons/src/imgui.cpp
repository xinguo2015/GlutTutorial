...
namespace xglm {
	...
	{
		int alignX = 0;
		int alignY = (h-guiFont.mFontHeight)/2;
		...
		if (mGuiState.hotitem == id)    {
			...
			if ( ! mGuiState.mousedown ) {
				...
				// 添加按钮的标签
				drawText(label, x+alignX, y+alignY,guiColorLabel);
			} else {
				// mouse is down，将按钮的位置进行稍许偏移，增加动感
				fillRect(x+guiButtonSink, y-guiButtonSink, w, h, guiColorHot);
				// 添加按钮的标签
				drawText(label, x+alignX+guiButtonSink, y-guiButtonSink+alignY, guiColorLabel);
			}
		} else {
			// button is not hot
