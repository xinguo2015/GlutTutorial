...
namespace xglm {
	...
	void GLUTView::setShape(Shape3D * shape)
	{
		...
		_modelview = glm::translate(glm::mat4(1.0f), glm::vec3(-eye.x, -eye.y, -eye.z));
	...
	void GLUTView::initLights()
	{
		...
		glLoadIdentity();
		glLightfv(GL_LIGHT0, GL_POSITION, glm::value_ptr( _values.get("GL_LIGHT0_POSITION", VarValue(Vec4f(0,0,0,1))).getVec4f() ));
		glLightfv(GL_LIGHT0, GL_DIFFUSE,  glm::value_ptr( _values.get("GL_LIGHT0_DIFFUSE",  VarValue(Vec4f(1,1,1,1))).getVec4f() ));
	}
