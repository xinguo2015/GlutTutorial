	...
	std::vector<Vec3f> & faceNormal )
{
	...
	{	
		...
		faceNormal[k] = glm::cross(position[v[1]]-position[v[0]], position[v[2]]-position[v[0]]);
	}
	return 1;
}
...
int TriMesh::needVtxNormals()
{
	...
	for( int k=0; k<VN; k++ )
		mNormal[k] = glm::normalize(mNormal[k]);
	return 1;
}
...
{	
	const int *v = glm::value_ptr( mTriangles[f] );
	return glm::cross(mPosition[v[1]]-mPosition[v[0]],
